...
prepare_turn(Turn, Dist, Street) -- ['prepare.ogg', 'after.ogg', D, M | Sgen] :- distance(Dist, dativ) -- D, turn(Turn, M), onto_street(Street, Sgen).
turn(Turn, Dist, Street) -- ['after.ogg', D, M, ' '| Sgen] :- distance(Dist, dativ) -- D, turn(Turn, M), onto_street(Street, Sgen).
...
roundabout(Dist, _Angle, Exit, Street) -- ['after.ogg', D, 'roundabout.ogg', 'then.ogg', 'take.ogg', E, 'exit.ogg' | Sgen] :- distance(Dist, dativ) -- D, nth(Exit, E), onto_street(Street, Sgen).
roundabout(_Angle, Exit, Street) -- ['take.ogg', E, 'exit.ogg' | Sgen] :- nth(Exit, E), onto_street(Street, Sgen).

go_ahead -- ['go_ahead.ogg'].
go_ahead(Dist, Street) -- ['follow1.ogg', D, 'follow2.ogg'| Sgen]:- distance(Dist, nominativ) -- D, to_street(Street, Sgen).
...
dist(D, [L|'20.ogg']) :-  D < 30, Ts is D - 20, !, distAnd(Ts, L).
dist(D, [L|'30.ogg']) :-  D < 40, Ts is D - 30, !, distAnd(Ts, L).
dist(D, [L|'40.ogg']) :-  D < 50, Ts is D - 40, !, distAnd(Ts, L).
dist(D, [L|'50.ogg']) :-  D < 60, Ts is D - 50, !, distAnd(Ts, L).
dist(D, [L|'60.ogg']) :-  D < 70, Ts is D - 60, !, distAnd(Ts, L).
dist(D, [L|'70.ogg']) :-  D < 80, Ts is D - 70, !, distAnd(Ts, L).
dist(D, [L|'80.ogg']) :-  D < 90, Ts is D - 80, !, distAnd(Ts, L).
dist(D, [L|'90.ogg']) :-  D < 100, Ts is D - 90, !, distAnd(Ts, L).
...
distAnd(X, [Ogg]) :- X < 10, !, num_atom(X, A), atom_concat(A, '_and.ogg', Ogg).
